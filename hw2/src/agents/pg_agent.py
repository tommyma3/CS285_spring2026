...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(len(rewards)):
            sum_rewards += self.gamma ** t * rewards[t]
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            for traj in rewards:
                ...
                traj_q_values = self._discounted_return(traj_list)
                traj_q_values = np.array(traj_q_values)
            ...
            for traj in rewards:
                traj_list = traj.tolist()
                traj_q_values = self._discounted_reward_to_go(traj_list)
                traj_q_values = np.array(traj_q_values)
                q_values.append(traj_q_values)
